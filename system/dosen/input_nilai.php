...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['nilai']) && is_array($_POST['nilai'])) {
        ...
        foreach ($_POST['nilai'] as $id_mahasiswa => $nilai) {
            if (!empty($nilai)) {
                $sql = "INSERT INTO penilaian (id_mahasiswa, id_matakuliah, nilai) 
                        VALUES ('$id_mahasiswa', '$id_matakuliah', '$nilai')
        ...
        if (empty($errors)) {
            echo "<div class='success-message'>Nilai berhasil disimpan.</div>";
        } else {
        ...
        echo "<div class='error-message'>Form nilai tidak valid atau kosong.</div>";
    }
}
?>
...
<body>
    <div class="header-text">
        Input Nilai Mahasiswa untuk Matakuliah ID: <?php echo htmlspecialchars($id_matakuliah); ?>
    </div>
    <form method="post">
        <table>
            <thead>
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td><input type='text' name='nilai[" . htmlspecialchars($row['id']) . "]' class='status-input'></td>";
                        echo "</tr>";
                    }
                } else {
